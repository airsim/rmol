#ifndef __RMOL_SERVICEABSTRACT_HPP
#define __RMOL_SERVICEABSTRACT_HPP
...
operator<< (std::basic_ostream<charT, traits>& ioOut,
            const RMOL::ServiceAbstract& iService) {
  /**
     string stream:
      - with same format
...
operator>> (std::basic_istream<charT, traits>& ioIn,
            RMOL::ServiceAbstract& ioService) {
  // Fill Service object with input stream
  ioService.fromStream (ioIn);
  return ioIn;
}

#endif // __RMOL_SERVICEABSTRACT_HPP
