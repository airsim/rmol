comfac_h_sources = $(top_srcdir)/latus/com/factory/FacBomAbstract.hpp \
		$(top_srcdir)/latus/com/factory/FacSupervisor.hpp \
		$(top_srcdir)/latus/com/factory/FacServiceAbstract.hpp \
		$(top_srcdir)/latus/com/factory/FacServiceContext.hpp \
		$(top_srcdir)/latus/com/factory/FacBookingDay.hpp \
		$(top_srcdir)/latus/com/factory/FacWholeDemand.hpp \
		$(top_srcdir)/latus/com/factory/FacCityPair.hpp \
		$(top_srcdir)/latus/com/factory/FacCityPairDate.hpp \
		$(top_srcdir)/latus/com/factory/FacWTP.hpp \
		$(top_srcdir)/latus/com/factory/FacWorldSchedule.hpp \
		$(top_srcdir)/latus/com/factory/FacInventory.hpp \
		$(top_srcdir)/latus/com/factory/FacFlightDate.hpp \
		$(top_srcdir)/latus/com/factory/FacLegDate.hpp \
		$(top_srcdir)/latus/com/factory/FacLegCabin.hpp \
		$(top_srcdir)/latus/com/factory/FacSegmentDate.hpp \
		$(top_srcdir)/latus/com/factory/FacSegmentCabin.hpp \
		$(top_srcdir)/latus/com/factory/FacTravelSolution.hpp \
		$(top_srcdir)/latus/com/factory/FacTravelSolutionBlock.hpp \
		$(top_srcdir)/latus/com/factory/FacNetwork.hpp \
		$(top_srcdir)/latus/com/factory/FacNetworkDate.hpp \
		$(top_srcdir)/latus/com/factory/FacAirportDate.hpp \
		$(top_srcdir)/latus/com/factory/FacOutboundPath.hpp
comfac_cc_sources = $(top_srcdir)/latus/com/factory/FacBomAbstract.cpp \
		$(top_srcdir)/latus/com/factory/FacSupervisor.cpp \
		$(top_srcdir)/latus/com/factory/FacServiceAbstract.cpp \
		$(top_srcdir)/latus/com/factory/FacServiceContext.cpp \
		$(top_srcdir)/latus/com/factory/FacBookingDay.cpp \
		$(top_srcdir)/latus/com/factory/FacWholeDemand.cpp \
		$(top_srcdir)/latus/com/factory/FacCityPair.cpp \
		$(top_srcdir)/latus/com/factory/FacCityPairDate.cpp \
		$(top_srcdir)/latus/com/factory/FacWTP.cpp \
		$(top_srcdir)/latus/com/factory/FacWorldSchedule.cpp \
		$(top_srcdir)/latus/com/factory/FacInventory.cpp \
		$(top_srcdir)/latus/com/factory/FacFlightDate.cpp \
		$(top_srcdir)/latus/com/factory/FacLegDate.cpp \
		$(top_srcdir)/latus/com/factory/FacLegCabin.cpp \
		$(top_srcdir)/latus/com/factory/FacSegmentDate.cpp \
		$(top_srcdir)/latus/com/factory/FacSegmentCabin.cpp \
		$(top_srcdir)/latus/com/factory/FacTravelSolution.cpp \
		$(top_srcdir)/latus/com/factory/FacTravelSolutionBlock.cpp \
		$(top_srcdir)/latus/com/factory/FacNetwork.cpp \
		$(top_srcdir)/latus/com/factory/FacNetworkDate.cpp \
		$(top_srcdir)/latus/com/factory/FacAirportDate.cpp \
		$(top_srcdir)/latus/com/factory/FacOutboundPath.cpp
