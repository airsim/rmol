#ifndef __LATUS_COM_SVC_SERVICEABSTRACT_HPP
#define __LATUS_COM_SVC_SERVICEABSTRACT_HPP
...
operator<< (std::basic_ostream<charT, traits>& ioOut,
            const LATUS::COM::ServiceAbstract& iService) {
  /**
     string stream:
     - with same format
...
operator>> (std::basic_istream<charT, traits>& ioIn,
            LATUS::COM::ServiceAbstract& ioService) {
  // Fill Service object with input stream
  ioService.fromStream (ioIn);
  return ioIn;
}

#endif // __LATUS_COM_SVC_SERVICEABSTRACT_HPP
