...
dnl $Id: boost.m4,v 1.22.2.4 2006/11/09 18:20:12 nihilus Exp $
...
if test x"$PKG_CONFIG" != x; then
	$PKG_CONFIG --exists boost && latus_boost_version=`$PKG_CONFIG --modversion boost | cut -d "." -f 1 | awk '{print $'0'".0"}'`
fi

if test x"${latus_boost_version}" = x; then
  ...
  for i in $pathlist; do
    for j in `ls -dr $i/boost* 2>/dev/null`; do
      if test -f ${j}/boost/detail/lightweight_mutex.hpp; then
        latus_boost_topdir=`basename $j`
        latus_boost_version=`echo ${latus_boost_topdir} | sed -e 's:boost-::'`
        break
      fi
    done
    if test x$latus_boost_version != x; then
  ...
  if test x"${latus_boost_version}" = x; then
   ...
   AC_MSG_RESULT(${latus_boost_version})
  fi
  ...
  version_suffix=`echo ${latus_boost_version} | tr '_' '.'`
  save_LIBS="$LIBS"
  AC_LANG_PUSH(C++)
